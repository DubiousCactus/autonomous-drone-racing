...
of view, and back-project its center coordinates onto the image, so that a
...
\begin{figure}[h]
	...
	\end{python}
	\caption[The look_at() function]{Creating the view matrix via the convenience
		\pyth{look_at()} function.}
\end{figure}
...
Once the camera is modeled and is located and oriented in the virtual scene so
that it matches the real world conditions, each randomly picked 3D mesh is
randomly translated and oriented in the scene. By applying random
transformations, the algorithm aims to recreate unique combinations of drone
racing circuits for each frame. As a result, it is possible, and often the
case, that only a small subset of the originally selected meshes are visible on
the image frame (depending on the camera pose), or even none at all. For
instance, if an extracted frame from the base dataset indicates that the drone
is facing a wall up close, it would make sense not to render any gates at all,
...
virtual world, that match the boundaries of the physical environment.\\
...
of the transformations), defined as
...
set to 0 as to keep the gate on the ground level. As for $M_R$, it is a $4
\times 4$ matrix inferred from a quaternion calculated by a rotation
around the $Z$-axis, as a random factor of Pi, so that it stays vertically
...
\todo{Describe how the scale is relative to the unitless OpenGL system?}

	\subsubsection{Back-projection onto the image frame}

- Describe how the closest gate is computed
- Describe how it is made sure that it is visible on screen
- Describe how I add an out-of-screen percentage of tolerance
- Describe how the back-projection is done to obtain pixel coordinates
