...
\tikzstyle{control}=[draw, node distance=3.5cm,draw, rectangle,fill=red!20, text
width=2.5cm,text centered,minimum width=1cm, minimum height=3cm]
...
%% Control Nodes
\node[control,node distance=4.5cm, minimum width=2cm,below of=cnn] (state) {State Machine};
\node[control,right of=state,minimum width=0.5cm,minimum height=1cm,yshift=1cm] (pid) {PID};
\node[control,right of=pid,minimum width=2cm,minimum height=1cm] (safety) {Safety Mechanism};
\node[control, node distance=2cm,below of=safety,minimum width=2cm,minimum height=1cm] (low) {Low Level Control};
...
\draw[thick] ($(safety.south west)!0.66!(safety.south east)$) --  ($(low.north
    west)!0.66!(low.north east)$);
\draw[thick] (median) -- ++(0,-1.5cm) -|($(pid.north west)!0.68!(pid.north east)$);
