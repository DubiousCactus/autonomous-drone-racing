\begin{tikzpicture}[>=stealth',shorten >=1pt,auto]
   ...
   % Draw the velocities
   \node[draw,rectangle,dashed,minimum width=4em,node distance=3cm,right
   of=takeOff, yshift=-3em] (velTakeOff) {$\mathbf{v}=(0,0,.5)$};
   \path[draw,dashed] (takeOff.east) -- (velTakeOff);

   \node[draw,rectangle,dashed,minimum width=4em,node distance=2cm,below
   right of=flying,yshift=-1em] (velFlying) {$\mathbf{v}=\texttt{PID()}$};
   \path[draw,dashed] (flying.south) -- (velFlying);

   \node[draw,rectangle,dashed,minimum width=4em,node distance=2cm,below
   left of=waiting] (velWaiting) {$\mathbf{v}=(0,0,0)$};
   \path[draw,dashed] (waiting.south) -- (velWaiting);

   \node[draw,rectangle,dashed,minimum width=4em,node distance=2cm,above
   left of=leaving,yshift=1em] (velLeaving) {$\mathbf{v}=(.5,0,0)$};
   \path[draw,dashed] (leaving.north) -- (velLeaving);

   \node[draw,rectangle,dashed,minimum width=4em,node distance=2.5cm,above
   right of=crossing] (velCrossing) {$\mathbf{v}=(.5,0,0)$};
   \path[draw,dashed] (crossing.north) -- (velCrossing);

   \node[draw,rectangle,dashed,minimum width=4em,node distance=2cm,above
   left of=aiming] (velAiming) {$\psi=0.05$};
   \path[draw,dashed] (aiming.north) -- (velAiming);
\end{tikzpicture}
